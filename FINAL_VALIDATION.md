...
## Runtime note
- The fixed-interactome proximity cell builds a ~15,640-node STRING graph; the full notebook
  executes cleanly, but on a 16 GB machine a repeated run can hit a memory-related kernel restart.
  Re-run with other memory-heavy applications closed.
...
- `results/network/node_info.tsv` is now generated by a dedicated notebook cell, which also
  rewrites the GraphML with node attributes (log2FC, padj, centralities, community, drug_target_for).
- A drug-target -> community mapping cell writes `results/network/drug_target_communities.csv`
  (nearest disease-module node and its Leiden community per mapped target) and
  `results/network/drug_target_paths.graphml` (targets + module + connecting shortest paths).
  This is descriptive: the module is small/exploratory and proximity is not significant.
